...
* **تنظيم الـ Profiles:** كل ميزة (Feature) يجب أن تملك Profile مستقل خاص بها (مثال: `SaleInvoiceMappingProfile`). لا للـ Profiles المركزية المجمعة.
...
## سابعاً: المجلدات وهيكلتها (Vertical Slices)
في طبقة الـ `Application` والـ `API` يتم بناء المجلدات بحسب الـ Feature وليس بحسب النمط التقني. 
مثلا مجلد مخصص لـ `CreateSaleInvoice` يكون داخله الـ Command و الـ Handler و الـ Validator و Profile الـ Mapper جميعاً.
